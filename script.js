...
function updateThemeButton(){
    ...
    document.querySelectorAll('.theme-btn-text').forEach(el => {
        el.innerText = theme==='dark' ? 'دۆخی ڕۆژ ☀️' : 'دۆخی شەو 🌙';
    });
}
...
function updateNetwork(isOnline){
    document.querySelectorAll('.badge-status').forEach(b=>{
        b.classList.toggle('online',isOnline);
        b.classList.toggle('offline',!isOnline);
        b.innerText=isOnline?'ئۆنلاین':'ئۆفلاین';
    });
}
...
window.onload=()=>{
    ...
    const saved=localStorage.getItem('electionUser');
...
function showDashboard(){
    ...
    const btnContact=document.getElementById('btnContactSup');
    if(center && center.supPhone!=="-"){btnContact.href=`https://wa.me/${center.supPhone}`;btnContact.classList.remove('hidden');}
...
function renderTimeChips(){const wrap=document.getElementById('timeChipWrap');if(!wrap||!currentUser)return;const submitted=JSON.parse(localStorage.getItem('votes_'+currentUser.username)||'[]').map(e=>e.timeValue);const cur=document.getElementById('timeSlot').value;wrap.innerHTML=Object.keys(timeLabels).map(k=>{const d=submitted.includes(k),a=cur===k;return`<button type="button" class="time-chip ${d?'chip-done':''} ${a?'chip-active':''}" onclick="selectTimeChip('${k}')">${d?'✓ ':''}${timeLabels[k]}</button>`;}).join('');}
...
function checkExistingData(){const t=document.getElementById('timeSlot').value,inp=document.getElementById('voterCount'),btn=document.getElementById('submitBtn');if(!t)return;inp.disabled=false;btn.classList.remove('disabled');const data=JSON.parse(localStorage.getItem('votes_'+currentUser.username))||[],entry=data.find(e=>e.timeValue===t);if(entry){inp.value=entry.count;btn.querySelector('.btn-inner').innerText='نوێکردنەوە';btn.querySelector('.btn-inner').style.background='linear-gradient(135deg, #F4B860 0%, #D4A04A 100%)';btn.querySelector('.btn-inner').style.color='#1a1a1a';calculateLivePercentage();}else{inp.value='';document.getElementById('livePercentage').innerText='';document.getElementById('liveGaugeWrap').innerHTML='';btn.querySelector('.btn-inner').innerText='ناردنی داتا';btn.querySelector('.btn-inner').style.background='linear-gradient(135deg, #1E7A46 0%, #0d5a2e 100%)';btn.querySelector('.btn-inner').style.color='#fff';}}

function submitData(){const t=document.getElementById('timeSlot').value,v=parseInt(document.getElementById('voterCount').value),msg=document.getElementById('submitMessage');if(!t)return showMessage(msg,'کاتێک هەڵبژێرە.','error');if(isNaN(v)||v<0||v>450)return showMessage(msg,'ژمارەی دروست بنووسە (٠-٤٥٠).','error');let data=JSON.parse(localStorage.getItem('votes_'+currentUser.username))||[];const prevMax=data.filter(e=>parseInt(e.timeValue)<parseInt(t)).reduce((m,e)=>Math.max(m,e.count),0);const nextMin=data.filter(e=>parseInt(e.timeValue)>parseInt(t)).reduce((m,e)=>Math.min(m,e.count),450);if(v<prevMax)return showMessage(msg,`هەڵە: نابێت کەمتر بێت لە کاتی پێشوو (${prevMax})`,'error');if(v>nextMin)return showMessage(msg,`هەڵە: نابێت زیاتر بێت لە کاتی دواتر (${nextMin})`,'error');const idx=data.findIndex(e=>e.timeValue===t);if(idx>-1)data[idx].count=v;else data.push({timeValue:t,timeLabel:timeLabels[t],count:v});data.sort((a,b)=>parseInt(a.timeValue)-parseInt(b.timeValue));localStorage.setItem('votes_'+currentUser.username,JSON.stringify(data));showMessage(msg,'سەرکەوتووبوو!','success');checkExistingData();renderTimeChips();loadStationData();}
...
function renderCentersConfig(){
    ...
    configDiv.style.cssText='background:var(--color-card);border:1px solid var(--color-border);border-radius:12px;padding:20px;margin-top:28px;';
    configDiv.innerHTML=`<h4 style="color:var(--color-primary);margin:0 0 16px 0;font-size:17px;font-weight:700;">⚙️ ڕیكخستنی زانیاری لێپرسراونی بنكه‌كان</h4><div id="centersConfigList" style="max-height:400px;overflow-y:auto;padding-right:10px;"></div>`;
    const adminBox=document.querySelector('.admin-box');
    if(adminBox){
        const existing=adminBox.querySelector('[id="centersConfigList"]');
        if(existing)existing.parentNode.parentNode.remove();
        adminBox.appendChild(configDiv);
    }
    const configList=document.getElementById('centersConfigList');
...
function onFilterChange(lvl){const cir=document.getElementById('filterCircle').value,cenS=document.getElementById('filterCenter'),staS=document.getElementById('filterStation');if(lvl==='circle'){cenS.innerHTML='<option value="all">هەموو بنکەکان</option>';staS.innerHTML='<option value="all">هەموو وێستگەکان</option>';if(cir!=='all')centersData.filter(c=>c.circle===cir).forEach(c=>cenS.innerHTML+=`<option value="${c.name}">${c.name}</option>`);}else if(lvl==='center'){staS.innerHTML='<option value="all">هەموو وێستگەکان</option>';if(cenS.value!=='all')for(let i=1;i<=5;i++)staS.innerHTML+=`<option value="وێستگەی ${i}">وێستگەی ${i}</option>`;}calculateFilteredStats();loadAdminTimeData();}

function calculateFilteredStats(){const cir=document.getElementById('filterCircle').value,cen=document.getElementById('filterCenter').value,sta=document.getElementById('filterStation').value;let filt=users.filter(u=>u.role==='observer');if(cir!=='all')filt=filt.filter(u=>u.circle===cir);if(cen!=='all')filt=filt.filter(u=>u.center===cen);if(sta!=='all')filt=filt.filter(u=>u.station===sta);let total=0;filt.forEach(u=>{const d=JSON.parse(localStorage.getItem('votes_'+u.username))||[];if(d.length)total+=Math.max(...d.map(e=>e.count));});const p=filt.length?((total/(filt.length*450))*100).toFixed(2):0;document.getElementById('adminFilteredVotes').innerText=total;document.getElementById('adminFilteredPercentage').innerText=`${p}%`;renderGauge('adminGaugeWrap',p);const cont=document.getElementById('circleCompareChart');cont.innerHTML='';[...new Set(centersData.map(c=>c.circle))].forEach(c=>{const us=users.filter(u=>u.role==='observer'&&u.circle===c);let tot=0;us.forEach(u=>{const d=JSON.parse(localStorage.getItem('votes_'+u.username))||[];if(d.length)tot+=Math.max(...d.map(e=>e.count));});const per=us.length?((tot/(us.length*450))*100).toFixed(1):0;cont.innerHTML+=`<div class="compare-row"><div class="compare-label">${c}</div><div class="compare-track"><div class="compare-fill" style="width:${per}%;"></div></div><div class="compare-value">${per}%</div></div>`;});}

function loadAdminTimeData(){
    ...
    let cen=document.getElementById('filterCenter').value;
    let sta=document.getElementById('filterStation').value;
    ...
    let mC=0,sC=0;
    filt.forEach(u=>{
        const ent=(JSON.parse(localStorage.getItem('votes_'+u.username))||[]).find(e=>e.timeValue===t);
        if(ent){
            sC++;
